...
class AppleCatcher:
    ...
    def _gapple_hit(self):
        ...
        if self.stats.gapples_left > 0:
            self.stats.gapples_left -= 0
            self.music.apple_droped.play()
            self.sb.prep_gapples()
        else:
    ...
    def _drop_gapples(self):
       # deja caer la manzana en una pocision random
        ...
        if len(self.gapples) == 1:
            for gapple in self.gapples.sprites():
                if gapple.rect.bottom > 300:
                    new_gapple = Gapple(self)
                    self.gapples.add(new_gapple)
        if len(self.gapples) == 2:
            for gapple in self.gapples.sprites():
                if gapple.rect.bottom > 600:
                    new_gapple = Apple(self)
                    self.gapples.add(new_gapple)
        if len(self.gapples) == 3:
            for gapple in self.gapples.sprites():
                if gapple.rect.bottom > 900:
                    new_gapple = Apple(self)
                    self.gapples.add(new_gapple)
